...
def CaculateProcess(processNum):
    print("Process"+str(processNum)+" start")
    c=0.001
    p=0.8
    miu=1
    REPEAT_COUNT=500
    ...
    PERCENT10_COUNT=0
    P_divive_sum=1000
    P_LOW = int(1 + (processNum-1) * P_divive_sum/16)
    P_HIGH = int(processNum * P_divive_sum/16)
    P_STEP = 8
    C_LOW = 1
    C_HIGH = 1000
    C_STEP = 8
    ...
    for p in range(P_LOW,P_HIGH,P_STEP):
        ...
        for c in range(C_LOW,C_HIGH,C_STEP):
            ...
            for k in range(REPEAT_COUNT):
                X1 = []
                res = []
                itemss = int(random.random() * 500)+500
                xaxis = []
                re = 0
                B = []
    ...
    Data=np.array(Data)
    Data=Data.T
    with open('algorithm3_exp_data_'+str(processNum)+'.csv', mode='w', encoding='utf-8',newline='') as file:
    ...
    #plt.show()
    print("Process" + str(processNum) + " end")
...
def main():
    ...
    for i in range(16):
        data.append(i+1)
    # 使用map进行并行处理
    pool.map(CaculateProcess, [(item) for item in data])
    # 关闭进程池
    pool.close()
    pool.join()
